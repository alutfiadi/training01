sap.ui.define([
    ...
    "sap/m/MessageToast",
    "sap/m/MessageBox",
    "training01/model/formatter" // <-- change training01 with your app id
], function (Controller,
    History,
    Filter,
    FilterOperator,
    ...
    return Controller.extend("training01.controller.BaseController", { // <-- change training01 with your app id
        formatter: formatter,
        /**
        ...
        onNaviBack: function () {
            ...
            if (sPreviousHash !== undefined || !oCrossAppNavigator.isInitialNavigationAsync()) {
                history.go(-1);
            } else {
                this.getRouter().navTo("RouteHome", {}, bReplace);
            }
        },
         ...
         * @public
         */
        onSearch: function (sQuery, aFields, sId) {
            var oTableSearchState = [];

            if (sQuery && sQuery.length > 0) {
                aFields.forEach((item) => {
                    const operator = FilterOperator[item.type];
                    oTableSearchState.push(new Filter(item.field, operator, sQuery));
                })
            }
            ...
            this.getView().byId(sId).getBinding("items").filter(allFilter, "Application");
        },
         ...
         * @public
         */
        bindView: function (oView, oModel, sModelName, sEntityPath) {
            oView.setModel(oModel, sModelName);
            oView.bindElement({ path: sEntityPath, model: sModelName });
        }
